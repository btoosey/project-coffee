...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("character_focus_toggle") && hover_component.is_hovered:
		...
		if movement_component:
			movement_component.visual_path_line2D.default_color = movement_component.LINE_COLOR
	...
	if event.is_action_pressed("character_focus_toggle") && !hover_component.is_hovered:
		...
		if movement_component:
			movement_component.visual_path_line2D.default_color = Color.TRANSPARENT
